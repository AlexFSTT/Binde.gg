...
class _MatchPlayer {
  ...
  const _MatchPlayer({required this.id, required this.username, required this.team, required this.elo, this.avatarUrl,
    this.kills = 0, this.deaths = 0, this.assists = 0, this.adr = 0, this.hltvRating = 0, this.payout = 0, this.eloChange = 0});
}
      ...
      child: Column(children: [
        ...
        Padding(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 6),
          child: Row(children: [const SizedBox(width: 180),
            ...['K', 'D', 'A', 'ADR', 'HLTV', 'ELO±'].map((h) => SizedBox(width: 65, child: Text(h, textAlign: TextAlign.center,
...
class _PlayerStatRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(children: [
        ...
        _s(player.adr.toStringAsFixed(1)), _s(player.hltvRating.toStringAsFixed(2), c: _rc(player.hltvRating)),
  ...
  Widget _s(String v, {Color? c}) => SizedBox(width: 65, child: Text(v, textAlign: TextAlign.center,
