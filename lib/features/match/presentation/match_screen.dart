...
class _MatchScreenState extends State<MatchScreen> {
  ...
  Future<void> _loadMatch() async {
    ...
    if (result.isSuccess) {
      ...
      Log.d('Match loaded - status: ${_match!.status}, map: ${_match!.map}, connectString: ${_match!.connectString}');

      // Subscribe to realtime FIRST
      _subscribeRealtime();

      // Then check if we need to provision
      _checkAndProvision();
    } else {
  ...
  /// Check if server provisioning is needed and trigger it
  void _checkAndProvision() {
    if (_match == null) return;

    Log.d('_checkAndProvision: status=${_match!.status}, connect=${_match!.connectString}, provisioning=$_isProvisioning');

    if (_match!.status == 'ready_check' &&
        _match!.connectString == null &&
        !_isProvisioning) {
      Log.d('>>> TRIGGERING PROVISION');
      _provisionServer();
    }
  }
  ...
  Future<void> _loadPlayers() async {
    try {
      ...
      Log.d('Loaded ${_players.length} players');
    } catch (e) {
  ...
  void _subscribeRealtime() {
    ...
    _realtime.subscribeMatch(
      ...
      onMatchUpdate: (data) {
        ...
        Log.d('Realtime update: ${oldStatus} → ${updated.status}, connect: ${updated.connectString}');
        ...
        // Status changed from veto to ready_check — need to provision!
        if (oldStatus == 'veto' && updated.status == 'ready_check') {
          Log.d('>>> Veto just completed! Checking provision...');
          _checkAndProvision();
        }
      },
      onVeto: (data) {
        Log.d('Veto event received: ${data['map_name']}');
      },
    );
  }
  ...
  Future<void> _provisionServer() async {
    if (_isProvisioning) {
      Log.d('Already provisioning, skipping');
      return;
    }

    Log.d('>>> _provisionServer() START for match: ${widget.matchId}');
    ...
    try {
      ...
      Log.d('>>> Edge function response status: ${response.status}');
      Log.d('>>> Edge function response data: ${response.data}');
      ...
      if (response.status != 200) {
        ...
        final errorMsg = errorData is Map
            ? (errorData['error'] ?? 'Server provisioning failed')
        ...
        Log.d('>>> Provision call succeeded! Waiting for realtime update with connect string...');
      }
    } catch (e, stack) {
  ...
  Widget build(BuildContext context) {
    ...
    // ── VETO PHASE: render veto as child ────────────
    if (_match!.status == 'veto') {
      return Scaffold(
        ...
        body: MapVetoScreen(
          matchId: widget.matchId,
          onVetoComplete: () {
            Log.d('VetoScreen signaled completion, reloading match...');
            _loadMatch();
          },
        ),
      );
        ...
        child: Column(
          ...
          children: [
            ...
            if (_isProvisioning) ...[
              ...
              _ProvisionErrorCard(error: _provisionError!, onRetry: _provisionServer),
              const SizedBox(height: 28),
            ],
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [
          AppColors.primary.withValues(alpha: 0.08), AppColors.bgSurface,
        ]),
...
class _ProvisionErrorCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: AppColors.dangerMuted, borderRadius: BorderRadius.circular(14),
      ...
      child: Column(children: [
        ...
        const SizedBox(height: 16),
        ElevatedButton.icon(
          onPressed: onRetry, icon: const Icon(Icons.refresh_rounded, size: 18),
    ...
    return Container(
      ...
      child: Column(children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
            color: AppColors.bgSurfaceActive, borderRadius: BorderRadius.circular(10),
    ...
    return Container(
      ...
      child: Row(children: [
        Expanded(child: Column(children: [
          ...
          Text('${match.teamAScore}', style: AppTextStyles.monoLarge.copyWith(
              fontSize: 48, color: match.winner == 'team_a' ? AppColors.success : AppColors.textPrimary)),
        ])),
        Column(children: [
          Text('VS', style: AppTextStyles.label.copyWith(color: AppColors.textTertiary, fontSize: 16)),
          if (match.map != null) ...[const SizedBox(height: 4),
        ...
        Expanded(child: Column(children: [
          ...
          const SizedBox(height: 4),
          Text('${match.teamBScore}', style: AppTextStyles.monoLarge.copyWith(
              fontSize: 48, color: match.winner == 'team_b' ? AppColors.success : AppColors.textPrimary)),
        ])),
      ]),
    );
  }
      ...
      child: Column(children: [
        ...
        Padding(
          ...
          child: Row(children: [
            ...
            ...['K', 'D', 'A', 'ADR', 'HLTV', 'ELO±'].map((h) => SizedBox(width: 65,
...
class _PlayerStatRow extends StatelessWidget {
        ...
        SizedBox(width: 180, child: Row(children: [
          Container(width: 28, height: 28,
            ...
            child: player.avatarUrl == null
                ? Center(child: Text(player.username[0].toUpperCase(),
                    style: AppTextStyles.caption.copyWith(color: teamColor, fontWeight: FontWeight.w700)))
                : null,
          ),
          const SizedBox(width: 8),
          Expanded(child: Text(player.username, style: AppTextStyles.bodySmall.copyWith(
              color: AppColors.textPrimary, fontWeight: FontWeight.w500), overflow: TextOverflow.ellipsis)),
        ])),
  ...
  Widget _s(String v, {Color? c}) => SizedBox(width: 65,
      child: Text(v, textAlign: TextAlign.center, style: AppTextStyles.mono.copyWith(fontSize: 12, color: c ?? AppColors.textSecondary)));
