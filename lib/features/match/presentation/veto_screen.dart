...
/// Subscribes to real-time updates on match + map_vetoes tables.
class MapVetoScreen extends StatefulWidget {
  final String matchId;
  const MapVetoScreen({super.key, required this.matchId});
...
class _MapVetoScreenState extends State<MapVetoScreen> {
  ...
  String? _selectedMap; // final map
  String _matchStatus = 'veto';
  ...
  void dispose() {
    ...
    _realtime.unsubscribe('vetoes:${widget.matchId}');
    super.dispose();
  }
  ...
  Future<void> _loadData() async {
    try {
      ...
      _matchStatus = matchData['status'] as String;
      ...
      // 5. Check if veto already complete
      if (_matchStatus != 'veto') {
        _handleVetoComplete();
        return;
      }
  ...
  void _subscribeRealtime() {
    // Subscribe to match updates (turn changes, veto complete)
    _realtime.subscribeMatch(
      matchId: widget.matchId,
      onMatchUpdate: (data) {
        if (!mounted) return;
        setState(() {
          _matchStatus = data['status'] as String? ?? _matchStatus;
          _currentTurnTeam = data['veto_turn_team'] as String?;
          _selectedMap = data['map'] as String?;
      ...
      onVeto: (data) {
        if (!mounted) return;
        setState(() {
          _bannedMaps.add(data['map_name'] as String);
        });
      },
    );
  }

  void _handleVetoComplete() {
    // Give a moment to show the final map, then redirect
  ...
  Widget build(BuildContext context) {
    ...
    // Veto complete — show selected map
    if (_selectedMap != null) {
      return _VetoCompleteView(
              ...
              children: [
                ...
                Text(
                  ...
                  style: AppTextStyles.bodyMedium.copyWith(
                    color:
                        _isMyTurn ? AppColors.primary : AppColors.textTertiary,
                    fontWeight:
                        _isMyTurn ? FontWeight.w600 : FontWeight.w400,
                  ),
                ),
...
class _MapInfo {
  ...
  const _MapInfo(
      {required this.name, required this.displayName, this.imageUrl});
}
      ...
      body: Center(
        child: Column(
          ...
          children: [
            ...
            Text(
              mapInfo?.displayName ?? selectedMap,
